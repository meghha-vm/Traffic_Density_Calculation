...
public class policeViewPage extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_police_view_page);
        lv=(ListView) findViewById(R.id.lv);
        sh=getSharedPreferences("", Context.MODE_PRIVATE);
        url="http://"+sh.getString("ip","")+":5000/pView";
    ...
    private void UserApproval(){
        RequestQueue rq = Volley.newRequestQueue(policeViewPage.this);
                    ...
                    if (status.equalsIgnoreCase("success")){
                        ...
                        for(int i=0;i<jar.length();i++)
                        {
                           // Toast.makeText(getApplicationContext(), job.toString(), Toast.LENGTH_SHORT).show();
                            ...
                            licen.add(jo.getString("licenceNumber"));
...
//        };
    ...
    pos=position;
        Intent ii=new Intent(getApplicationContext(),policeDetails.class);
        ii.putExtra("pagetype","UserApproval");
        startActivity(ii);
